...
for f in $files; do
  case "$f" in
    .env|*/.env|*.env.*) # запрет на имя файла
...
```
Для сборки библиотеки по коммиту необходимо добавить post-commit хук:
```
#!/bin/sh
...
if [ "$BRANCH" = "dev" ]; then
    ...
    echo ">>> Библиотека собрана: build/src/libmylib.a"
fi

exit 0
```
`chmod +x .git/hooks/post-commit` - теперь хук исполняемый.
## 5. Автоматизация с помощью Github Actions
### YAML
__YAML__ - язык для хранения данных в читаемом формате, используется для конфигурационных файлов.
YAML описывает структуры данных:
- словари
- списки и массивы
- строки
- числа
...
### GitHub Actions
__GitHub Actions__ - встроенная система автоматизации CI/CD и workflow-процессов.
Позволяет запускать прямо в репозитории:
- сборку
- тестирование
- Workflows (сценарии CI/CD)
- деплой
- issue, PR, комментарии
- Runners (виртуальные машины)

Конфигурация задаётся файлами YAML в каталоге `.github/workflows`.   
__Тарифы GitHub Actions:__
Для публичных репозиториев - бесплатно без ограничений.
| Тариф| Цена|
|:---|:---|
| GitHub Free | $0|
| GitHub Pro| $4 / месяц |
| GitHub Team| $4 за пользователя / месяц|
| GitHub Enterprise Cloud | от $21 за пользователя / месяц |
### Определения CI-CD пайплайнов:
На ветке dev в папке `.github/workflows`:
```
name: CMake CI
on:
  push:
    branches: [dev, main] # при пуше в эти ветки
  pull_request:
    branches: [dev, main] # при пр в эти ветки
...
```
Теперь результат отображается во вкладке Actions в репозитории GitHub.  
Чтобы создать Workflow, токен доступа должен иметь право создавать такой тип файлов. это является отдельной настройкой.
